...
```bash
    az group create --name myLinux19 --location southeastasia
```

__Create Linux VM__
```bash
az vm create \
    --resource-group myLinux19 \
    --name myLinux19 \
    --image UbuntuLTS \
...
```bash
    az vm list-ip-addresses --resource-group myLinux19 --name myLinux19 -o table
```

__Setup NSG__
- List NSG
```bash
    az network nsg list --resource-group myLinux19 -o table
```

- List and Create NSG Rules
```bash
    az network nsg rule list --nsg-name myLinux19NSG --resource-group myLinux19

    az network nsg rule create -g myLinux19 --nsg-name myLinux19NSG -n nodeweb --priority 100 --destination-port-ranges 80
```
...
__Clean resources__
```bash
    az group delete --name myLinux19 \
    --no-wait --yes
```
